...
<head>
    ...
    <title>@yield('title', 'Hospital Panel - CovidCare')</title>
        ...
        <aside id="sidebar" class="fixed lg:relative z-50 w-72 h-full bg-white border-r border-gray-200 shadow-xl lg:shadow-none flex-shrink-0 -translate-x-full lg:translate-x-0 sidebar-transition overflow-y-auto scrollbar-hide">
            <div class="sticky top-0 bg-white/95 backdrop-blur-sm z-10 border-b border-gray-100 px-6 py-5">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-extrabold text-gray-900 tracking-tight">Hospital Panel</h1>
            ...
            <div class="px-4 py-4 border-b border-gray-100">
                <div class="flex items-center gap-3 px-3 py-2.5 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl border border-emerald-100">
                    <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-sm font-bold shadow-sm">
                        {{ strtoupper(substr(Auth::guard('hospital')->user()->hospital_name ?? 'H', 0, 1)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-gray-900 truncate">{{ Auth::guard('hospital')->user()->hospital_name ?? 'Hospital' }}</p>
                        <p class="text-xs text-gray-400 truncate">{{ Auth::guard('hospital')->user()->email }}</p>
                    </div>
            ...
            <nav class="px-3 py-4 space-y-1">
                @php
                    $navItems = [
                        ['route' => 'hospital.dashboard', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', 'label' => 'Dashboard'],
                        ['route' => 'hospital.requests', 'icon' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', 'label' => 'Manage Requests'],
                    ];
                @endphp
                @foreach($navItems as $item)
                    <a href="{{ route($item['route']) }}" 
                       class="sidebar-item flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50/80 transition-all duration-200 group {{ request()->routeIs($item['route']) ? 'active' : '' }}">
                        <svg class="sidebar-icon w-5 h-5 text-gray-400 group-hover:text-emerald-500 transition-colors {{ request()->routeIs($item['route']) ? 'text-emerald-500' : '' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 hidden sm:block">@yield('page-subtitle', 'Your facility hub')</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="relative p-2 hover:bg-gray-100 rounded-xl transition text-gray-500 hover:text-gray-700">
                                ...
                                @php
                                    $hUser = Auth::guard('hospital')->user();
                                    $hNotifCount = $hUser ? \App\Helpers\NotificationHelper::unreadCount($hUser) : 0;
                                @endphp
                                @if($hNotifCount > 0)
                                    <span class="absolute -top-0.5 -right-0.5 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{{ $hNotifCount > 9 ? '9+' : $hNotifCount }}</span>
                                @endif
                            </button>
                            <div x-show="open" @click.away="open = false" x-cloak
                                ...
                                <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                                    ...
                                    @if($hNotifCount > 0)
                                        <span class="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">{{ $hNotifCount }} new</span>
                                    @endif
                                </div>
                                <div class="max-h-72 overflow-y-auto divide-y divide-gray-50">
                                    @php $hNotifs = $hUser ? \App\Helpers\NotificationHelper::getRecent($hUser, 5) : collect(); @endphp
                                    @forelse($hNotifs as $notif)
                                        <div class="px-4 py-3 hover:bg-gray-50 transition {{ !$notif->is_read ? 'bg-emerald-50/30' : '' }}">
                                            <div class="flex items-start gap-3">
                                                <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5
                                                    @if(str_contains($notif->type, 'approved') || str_contains($notif->type, 'booked')) bg-emerald-100 text-emerald-600
                                                    ...
                                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        ...
                                                        @elseif(str_contains($notif->type, 'booked'))
                                                ...
                                                @if(!$notif->is_read)
                                                    <span class="w-2 h-2 bg-emerald-500 rounded-full shrink-0 mt-1.5"></span>
                                                @endif
                                            </div>
                                            @if($notif->link)
                                                <a href="{{ $notif->link }}" class="text-[10px] text-emerald-600 hover:underline mt-1 inline-block font-medium">View details &rarr;</a>
                                            @endif
                                        </div>
                                    @empty
                        ...
                        <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center text-emerald-700 text-xs font-bold">
                            {{ strtoupper(substr(Auth::guard('hospital')->user()->hospital_name ?? 'H', 0, 1)) }}
                        </div>
                    </div>
                </div>
